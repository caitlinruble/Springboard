...
SELECT DISTINCT CONCAT_WS(" ", firstname, surname) AS fullname,
	name AS facility,
	CASE WHEN b.memid = 0 
	THEN (slots * guestcost) 
	ELSE (slots * membercost) END AS session_cost
FROM Bookings as b
	INNER JOIN Members as m
		ON b.memid = m.memid
	INNER JOIN Facilities as f
...
SELECT * FROM (
	...
	WHERE EXTRACT(YEAR from starttime) = 2012 
		AND EXTRACT(MONTH from starttime) = 9 
		AND EXTRACT(DAY FROM starttime) = 14) AS allbookingcosts
WHERE session_cost >30
ORDER BY session_cost DESC
